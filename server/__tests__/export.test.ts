...
describe('CSV Export/Import', () => {
  ...
  describe('Export Data', () => {
    it('exports categories and time entries', () => {
      ...
      db.run(`INSERT INTO time_entries (user_id, category_id, note, start_time, end_time, duration_minutes) 
              VALUES (?, ?, ?, ?, ?, ?)`,
      ...
      const entriesResult = db.exec(`
        SELECT c.name, c.color, te.note, te.start_time, te.end_time, te.duration_minutes
  ...
  describe('Import Data', () => {
    it('imports time entries and creates missing categories', () => {
      ...
      const csvData = `Category,Color,Note,Start Time,End Time,Duration (minutes)
Development,#007bff,Working on feature,2024-01-15T09:00:00Z,2024-01-15T10:30:00Z,90`;
      ...
      const [categoryName, color, note, startTime, endTime] = dataRow;
      ...
      db.run(`INSERT INTO time_entries (user_id, category_id, note, start_time, end_time, duration_minutes) 
              VALUES (?, ?, ?, ?, ?, ?)`,
        [testUserId, categoryId, note, startTime, endTime, duration]);
