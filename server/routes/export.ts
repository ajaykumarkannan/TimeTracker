...
router.post('/csv/preview', (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!columnMapping) {
      ...
      const categoryIndex = lowerHeader.findIndex(h => h.includes('category') || h.includes('project'));
      const taskNameIndex = lowerHeader.findIndex(h => h.includes('task') || h.includes('description') || h.includes('note') || h.includes('comment'));
      ...
      if (taskNameIndex >= 0) autoMapping.taskName = taskNameIndex;
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const taskName = columnMapping.taskName !== undefined ? row[columnMapping.taskName]?.trim() || null : null;
...
router.post('/csv', (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!mapping) {
      ...
      if (header.length < 4) {
        return res.status(400).json({ error: 'Invalid CSV format. Expected columns: Category, Color, Task, Start Time, End Time' });
      }
      mapping = { category: 0, color: 1, taskName: 2, startTime: 3, endTime: 4 };
    }
    ...
    const entriesToImport = overrideEntries || lines.slice(1).map((line, index) => {
      ...
      return {
        ...
        task_name: mapping.taskName !== undefined ? row[mapping.taskName]?.trim() || null : null,
    ...
    for (const entry of entriesToImport) {
      ...
      const { category, color, task_name, startTime, endTime } = entry;
